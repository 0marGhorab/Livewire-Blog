    ...
    @php
        ...
        if (isset($posts) && $posts->count() > 0) {
            foreach($posts as $post) {
                ...
                echo "Post #{$post->id}: {$post->title}<br>";
                echo "Images count: {$post->images->count()}<br>";
                if ($post->images->count() > 0) {
                    echo "First image path: {$post->images->first()->path}<br>";
                }
                echo "</div>";
            }
        }
                ...
                @if($post->images && $post->images->count() > 0)
                    <div class="mb-4">
                        @if($post->images->count() === 1)
                            <!-- Single image -->
                            <div class="w-full overflow-hidden rounded-lg bg-gray-100" style="max-height: 400px;">
                                <img 
                        ...
                        @else
                            ...
                            @if($post->images->count() > 3)
                                <div class="mt-2 text-center">
                                    <span class="text-sm text-gray-600">+{{ $post->images->count() - 3 }} more images</span>
                                </div>
                            @endif
                        @endif
                    </div>
                    ...
                    @if ($post->user_id === auth()->id())
                        <div class="flex space-x-2">
                            ...
                            <a href="{{ route('posts.analytics', $post) }}" 
                               class="inline-flex items-center gap-2 bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition">
