    ...
    <div class="space-y-4">
        @forelse ($posts as $post)
            <div class="border border-brand-primary/20 rounded-xl p-10 bg-white/80 hover:shadow-lg transition-all duration-300 animate-fade-in-up"
                ...
                <p class="text-brand-primary/90 mb-3">{{ Str::limit($post->body, 150) }}</p>
                    ...
                    @if ($post->user_id === auth()->id())
                        <div class="flex space-x-2">
                            ...
                            <a href="{{ route('posts.analytics', $post) }}"
                               class="inline-flex items-center gap-2 bg-brand-button-secondary text-white px-3 py-1 rounded-lg hover:bg-brand-button-secondary-hover transition">
        ...
        @empty
            <p class="text-brand-primary/80 text-center py-8 animate-fade-in">No posts yet. Create the first one!</p>
        @endforelse
    </div>
